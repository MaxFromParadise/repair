(() => {
    ...
    const burger = document.querySelector(".icon-menu"), navbar = document.querySelector(".navbar");
    ...
    function script_extend(custom, defaults) {
        for (const key in defaults) if (null == custom[key]) {
    ...
    const MutationObserver = window.MutationObserver || window.WebkitMutationObserver || window.MozMutationObserver || class MutationObserver {
        constructor() {
    ...
    const script_getComputedStyle = window.getComputedStyle || function getComputedStyle(el) {
    ...
    class WOW {
        ...
        constructor(options = {}) {
            ...
            this.scrolled = true;
            this.config = script_extend(options, this.defaults);
        ...
        start() {
            ...
            if (this.boxes.length) if (this.disabled()) this.resetStyle(); else for (let i = 0; i < this.boxes.length; i++) {
        ...
        doSync(element) {
            if ("undefined" === typeof element || null === element) ({element} = this);
        ...
        animate=function animateFactory() {
            if ("requestAnimationFrame" in window) return callback => window.requestAnimationFrame(callback);
        ...
        vendorSet(elem, properties) {
            for (const name in properties) if (properties.hasOwnProperty(name)) {
        ...
        vendorCSS(elem, property) {
            const style = script_getComputedStyle(elem);
        ...
        animationName(box) {
            ...
            try {
                ...
                aName = script_getComputedStyle(box).getPropertyValue("animation-name");
            }
